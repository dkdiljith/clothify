document.addEventListener("DOMContentLoaded", function () {
  ...
  addToWishlistButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      const variationIndex = event.currentTarget.getAttribute(
  ...
  async function addToCart() {
    ...
    try {
      const response = await fetch(
        `/user/cart/${productId}/${variationIndex}/${quantity}`,
        {
          method: "POST",
  ...
  async function addToWishlist(productId, variationIndex) {
    ...
    const wasWishlisted = button.classList.contains("active");
    // Optimistic UI
    updateWishlistButton(button, !wasWishlisted);
    try {
      let response;
      if (wasWishlisted) {
        response = await fetch(`/user/removeFromWishlist/${productId}`, {
          method: "DELETE",
        });
      } else {
      ...
      if (!data.success) {
        // Rollback
        updateWishlistButton(button, wasWishlisted);
        showPopupMessage(data.message, data.info ? "info" : "error");
        return;
      }
      if (wasWishlisted) {
